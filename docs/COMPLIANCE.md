...
- **PHI-specific identifiers.** This tool targets GDPR "personal data" and the PII–PHI overlap (names, SSN, phone, email, IBAN, CC, organizations, locations). PHI-only HIPAA identifier categories — medical record numbers, health-plan IDs, account numbers, certificate/license numbers, NPI, vehicle identifiers, device identifiers, biometric identifiers, full-face photos — are **out of scope** at v0.1 and not on the [roadmap](roadmap.md). For HIPAA Safe Harbor de-identification of clinical notes use [philter](https://github.com/BCHSI/philter-ucsf) or Presidio's medical recognizers. See [GLOSSARY.md § PII vs PHI](GLOSSARY.md#pii-vs-phi) and [landscape/de-identification.md](landscape/de-identification.md).
