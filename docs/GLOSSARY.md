...
| **Codice Fiscale** | (Italian fiscal code) | Italy's 16-char alphanumeric tax/identity code; detected opt-in (`--detectors eu`) as `it_cf`, checksum-validated. | [USAGE.md](USAGE.md) |
...
| **DNI** | Documento Nacional de Identidad | Spanish national ID number; detected opt-in (`--detectors eu`) as `es_dni` (also covers **NIE**), checksum-validated. | [USAGE.md](USAGE.md) |
...
| **INSEE** | Institut national de la statistique et des études économiques | French agency that **issues** the NIR. INSEE is the agency; the **NIR** is the identifier (detected as type `fr_nir`). | [USAGE.md](USAGE.md) |
...
| **NIE** | Número de Identidad de Extranjero | Spanish ID for foreign residents; detected alongside DNI as type `es_dni`. | [USAGE.md](USAGE.md) |
| **NIR** | Numéro d'inscription au répertoire | French social-security number (the identifier INSEE issues); detected opt-in (`--detectors eu`) as `fr_nir`, checksum-validated. | [USAGE.md](USAGE.md) |
...
| **Steuer-ID** | Steuerliche Identifikationsnummer | German 11-digit personal tax ID; detected opt-in (`--detectors eu`) as `de_steuer`, checksum-validated. | [USAGE.md](USAGE.md) |
